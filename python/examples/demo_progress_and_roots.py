            ...
            async for msg in read_stream:
                ...
                if method == "initialize":
                    ...
                    async def fetch_roots():
                        ...
                        await write_stream.send(
                            JSONRPCRequest(
                                id="srv-roots-1", method="roots/list", params={}
                            )
                        )
                        roots_res = await fut
                        self.client_roots = roots_res.get("roots", [])
                        print(
                            f"   📂 [Server] Successfully discovered Client roots: {self.client_roots}\n"
                        )
                ...
                # Tool call (triggers progress streaming)
                    ...
                    if tool_name == "stream_download_task":
                        total_steps = 5
                        token = params.get("_meta", {}).get(
                            "progressToken", "dl-token-1"
                        )
                        ...
                        await write_stream.send(
                            JSONRPCNotification(
                                ...
                                params={
                                    "level": "info",
                                    "logger": "downloader",
                                    "data": "Starting 5-step data download...",
                                },
                            )
                        )
                            ...
                            JSONRPCResponse(
                                ...
                                result={
                                    "content": [
                                        {
                                            "type": "text",
                                            "text": "Download and indexing completed 100%!",
                                        }
                                    ]
                                },
                            )
...
def render_progress_bar(progress: int, total: int, width: int = 25):
    ...
    sys.stdout.write(
        f"\r   📊 [Client Live Stream] [{bar}] {percent}% (Step {progress}/{total})"
    )
    sys.stdout.flush()
    if progress == total:
        sys.stdout.write("\n")
    ...
    async with websockets.serve(ws_handler, "localhost", 8769):
        ...
        async with WebSocketClientTransport(url) as (read_stream, write_stream):
            ...
            if roots_req.get("method") == "roots/list":
                print(
                    f"   📥 [Client] Received Server-Initiated request: `roots/list` (id: {roots_req.get('id')})"
                )
